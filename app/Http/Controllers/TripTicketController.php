...
class TripTicketController extends Controller
{
    ...
    private function assertDriverNotArchived(int $driverId): ?JsonResponse
    {
        ...
        if ($driver && $driver->is_archived) {
            return response()->json([
                'message' => "This driver has been archived and can no longer be assigned to trips. Please select an active driver.",
                'errors'  => ['driver_id' => ["This driver has been archived and can no longer be assigned to trips."]],
            ], 422);
        }
        return null;
    }
    ...
    public function index()
    {
        try {
            ...
            return back()->withErrors(['error' => 'We couldn\'t load the trip tickets. Please refresh and try again.']);
        }
    }
    public function store(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
                ...
                'trip_no.unique'         => 'This trip number is already in use. Please use a different one.',
                'driver_id.required'     => 'Please select a driver for this trip.',
                'driver_id.exists'       => 'The selected driver could not be found. Please try again.',
                'truck_id.required'      => 'Please select a truck for this trip.',
                'truck_id.exists'        => 'The selected truck could not be found. Please try again.',
                'departure_time.date_format' => 'Departure time format is invalid. Please re-enter the time.',
                'arrival_time.date_format'   => 'Arrival time format is invalid. Please re-enter the time.',
            ...
            if ($truck && $truck->status !== 'Available') {
                return response()->json([
                    'message' => "Truck {$truck->truck_code} is currently unavailable ({$truck->status}). Please select a different truck.",
                    'errors'  => ['truck_id' => ["Truck {$truck->truck_code} is currently {$truck->status} and cannot be assigned."]],
                ], 422);
            }
            $driver = Driver::find($validated['driver_id']);
            if ($driver && $driver->status !== 'Available') {
                return response()->json([
                    'message' => "This driver is currently unavailable ({$driver->status}). Please select a different driver.",
                    'errors'  => ['driver_id' => ["This driver is currently {$driver->status} and cannot be assigned."]],
                ], 422);
            }
            ...
            return response()->json([
                'message' => 'We couldn\'t create the trip ticket right now. Please try again in a moment.',
            ], 500);
        }
    }
    ...
    {
        try {
            $trip        = TripTicket::with('driver')->findOrFail($id);
            ...
            $validated = $request->validate([
                ...
                'trip_no.required'       => 'Trip number is required.',
                'trip_no.unique'         => 'This trip number is already in use. Please use a different one.',
                'driver_id.required'     => 'Please select a driver for this trip.',
                'driver_id.exists'       => 'The selected driver could not be found. Please try again.',
                'truck_id.required'      => 'Please select a truck for this trip.',
            ...
            return response()->json([
                'message' => 'This trip ticket no longer exists or may have already been removed.',
            ], 404);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json([
                'message' => 'Some fields need your attention. Please review and try again.',
...
//----------Search/Filter----------------
    public function search(Request $request): JsonResponse
    {
        try {
            ...
            if ($query === '') {
                return response()->json(['message' => 'Please enter a trip number, driver, truck, origin, or destination to search.'], 422);
            }
            ...
            if ($trips->isEmpty()) {
                return response()->json([
                    'message' => "No trip tickets found matching \"{$query}\". Try a different keyword.",
                    'data'    => [],
                ], 404);
            }
    ...
    public function filterByStatus(Request $request): JsonResponse
    {
        try {
            ...
            if ($trips->isEmpty()) {
                ...
                return response()->json([
                    'message' => "No trip tickets found with status: {$label}.",
                    'data'    => [],
                ], 404);
            }
            return response()->json(['data' => $trips], 200);
    ...
    public function transition(Request $request, int $id): JsonResponse
    {
        try {
            ...
            if (!isset($allowed[$current]) || !in_array($validated['status'], $allowed[$current], true)) {
                return response()->json([
                    'message' => "This trip can't be moved to \"{$validated['status']}\" from its current status \"{$current}\".",
            ...
            if ($validated['status'] === 'In-Transit') {
                ...
                if ($truck && $truck->status !== 'Available') {
                    return response()->json([
                        'message' => "Truck {$truck->truck_code} is currently unavailable ({$truck->status}) and cannot be dispatched.",
                        'errors'  => ['truck_id' => ["Truck {$truck->truck_code} is currently {$truck->status}."]],
                    ], 422);
                }
                $driver = Driver::find($trip->driver_id);
                if ($driver && $driver->status !== 'Available') {
                    return response()->json([
                        'message' => "This driver is currently unavailable ({$driver->status}) and cannot be dispatched.",
                        'errors'  => ['driver_id' => ["Driver is currently {$driver->status}."]],
                    ], 422);
                }
            }
            $oldStatus = $trip->status;
            ...
            return response()->json([
                'message' => 'We couldn\'t update the trip status right now. Please try again in a moment.',
            ], 500);
        }
    }
    public function archive(Request $request, int $id): JsonResponse
    {
        try {
            ...
            return response()->json([
                'message' => 'We couldn\'t archive this trip ticket right now. Please try again in a moment.',
            ], 500);
        }
    }
    public function unarchive(Request $request, int $id): JsonResponse
    {
        try {
            ...
            return response()->json([
                'message' => 'We couldn\'t restore this trip ticket right now. Please try again in a moment.',
            ], 500);
        }
    }
    public function destroy(Request $request, int $id): JsonResponse
    {
        try {
            ...
            return response()->json([
                'message' => 'We couldn\'t delete this trip ticket right now. Please try again in a moment.',
            ], 500);
        }
    }
    // -------------------------------------------------------------------------
    ...
    private function assertTruckNotUnderMaintenance(int $truckId): ?JsonResponse
    {
        ...
        $message = "Truck {$code} can't be assigned right now — it has an active maintenance record ({$status}). "
                 . "Please complete or cancel that maintenance record first.";
        return response()->json([
            'message' => $message,
