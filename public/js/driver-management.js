...
function initializeEventListeners() {
    ...
    if (addDriverBtn) addDriverBtn.addEventListener('click', () => openModal('addDriverModal'));

    const archivedBtn = document.getElementById('archivedBtn');
    if (archivedBtn) archivedBtn.addEventListener('click', loadArchivedDrivers);
    ...
    window.FormValidation?.setupLicenseExpiryField('addLicenseExpiry',  'addLicenseExpiryPicker',  'addLicenseExpiryCalendarBtn');
    window.FormValidation?.setupLicenseExpiryField('editLicenseExpiry', 'editLicenseExpiryPicker', 'editLicenseExpiryCalendarBtn');
    ...
    if (editDriverForm) editDriverForm.addEventListener('submit', handleEditDriver);
...
async function viewDriver(driverId) {
    try {
        ...
        if (archivedModal?.classList.contains('show')) closeModal('archivedModal');
...
async function editDriver(driverId) {
    try {
        ...
        if (editExpiryInput) {
            ...
            if (editExpiryPicker) {
                editExpiryPicker.value = window.FormValidation?.displayDateToIso(editExpiryInput.value) || '';
            }
        }
...
async function handleAddDriver(e) {
    ...
    const form   = e.target;
    const isValid = window.FormValidation?.validateDriverForm(form) ?? true;
    if (!isValid) {
    ...
    if (!fileInput?.files?.length) {
        setFileUploadError('fileUploadArea', 'addDriverError', 'addDriverErrorList',
            'Please upload a driver file before submitting.');
        return;
    }
    clearFileUploadError('fileUploadArea', 'addDriverError');
    ...
    try {
        ...
        if (response.ok) {
            ...
            showFormError('addDriverError', 'addDriverErrorList',
                data?.errors ?? [data?.message || 'Something went wrong. Please try again.']);
        }
    } catch (error) {
...
async function confirmArchiveAction() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const data = await response.json().catch(() => null);
            showPasswordError('archivePasswordError', 'archivePasswordErrorText',
...
async function confirmUnarchiveAction() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const data = await response.json().catch(() => null);
            showPasswordError('unarchivePasswordError', 'unarchivePasswordErrorText',
                data?.message || 'Incorrect password. Please try again.');
        }
    } catch (error) {
...
function handleFileSelect(e) {
    ...
    if (file) clearFileUploadError('fileUploadArea', 'addDriverError');
...
function updateFileUploadUI(areaId, file) {
    if (!file) { resetFileUploadUI(areaId); return; }
...
/**
 * Show a file-upload error by routing it to the form's error panel.
 * @param {string} areaId      - ID of the file-upload area div
 * @param {string} boxId       - ID of the error panel (e.g. 'addDriverError')
 * @param {string} listId      - ID of the error list inside the panel
 * @param {string} message     - Error message to display
 */
function setFileUploadError(areaId, boxId, listId, message) {
    const area = document.getElementById(areaId);
    if (area) area.style.borderColor = '#dc2626';
    showFormError(boxId, listId, [message]);
}

function clearFileUploadError(areaId, boxId) {
    const area = document.getElementById(areaId);
    if (area) area.style.borderColor = '';
    clearFormError(boxId);
}

function initFileUploadAreaDefaults(areaId) {
    const area = document.getElementById(areaId);
    if (!area || (area.dataset.defaultText && area.dataset.defaultIcon)) return;
    const icon = area.querySelector('.material-symbols-outlined');
    const text = area.querySelector('p');
